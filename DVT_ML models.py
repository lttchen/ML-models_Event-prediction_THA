...
def ANN_model(hp):
    ...
    if hp_layers != 0:
        for i in range(hp_layers):
            model.add(tf.keras.layers.Dense(
                    ...
                    units=hp.Int(f"units_{i+1}", min_value=20, max_value=36, step=4),
    ...
    model.add(tf.keras.layers.Dropout(hp.Float("dropout_", 0, 0.2, step=0.05))) #adjust dropout - randomly setting a fraction rate of inputs to 0 to prevent overfitting
...
def RF(x_tune,y_tune,x_train,y_train,x_test, y_test, search)
    ...
    parameter_space = {
        'n_estimators':np.arange(5,100,5), 'min_samples_leaf':range(2,50,2)}
    ...
    rf_model=RandomForestClassifier(n_estimators=best_params['n_estimators'], min_samples_leaf=best_params['min_samples_leaf'], 
                max_features=28, bootstrap=False)
...
def HGB(x_tune,y_tune,x_train,y_train,x_test, y_test, search):
    ...
    parameter_space = {'learning_rate': [0.001,0.01,0.1, 0.5],
        'max_iter':np.arange(80,140,20), 'max_leaf_nodes': [31, 36, 41, 46, 51, 56]}
